        ...
        for(cNr = 1 ; cNr <= 5 ; cNr++){
            for(pNr = 1 ; pNr <= 5 ; pNr++){
                ...
                for(cI = 0 ; cI < (cNr-1) ; cI++)
                    c[cI] = new Thread(new Consumer(b, toGet));
                c[cI] = new Thread(new Consumer(b, toGet + toGetRest));

                for(pI = 0 ; pI < (pNr-1) ; pI++)
                    p[pI] = new Thread(new Inserter(b, toPut));
                p[pI] = new Thread(new Inserter(b, toPut + toPutRest));
