...
class InvalidAccount extends Exception {
    public final int id;
    public InvalidAccount (int id) {this.id=id;} 
...
class Account {
    ...
    public float balance () {return bal;}
    public void withdraw (float amount) {bal-=amount;}
    public void deposit (float amount) {bal+=amount;}
  }
...
//o professor gostaria mais que aqui tivesse reentrant lock();
  ...
  public void transfer (int from, int to, float amount) throws InvalidAccount, NotEnoughFunds{
  	...
  	if (f.closed || t.closed) {
  		f.unlock();
  		t.unlock(); //a ordem de unlock nao é importante
  ...
  public float totalBalance (int accounts []) throws InvalidAccount {
  	...
  	synchronized (this){
  		for (int i=0; i<id.length; i++){
  			ac[i] = map.get(id[i]);
  			if (ac[i]=null) throw new InvalidAccount (id[i]);
  		}
  	}
